...
void ZeissMTB_Erlangen::setPosition(POINT3 position) {
	...
	if (m_stageX && m_stageY) {
		success = m_stageX->SetPosition(position.x, "�m", MTBCmdSetModes::MTBCmdSetModes_Synchronous, 500);
		success = m_stageY->SetPosition(position.y, "�m", MTBCmdSetModes::MTBCmdSetModes_Synchronous, 500);
	}
	if (m_ObjectiveFocus) {
		success = m_ObjectiveFocus->SetPosition(position.z, "�m", MTBCmdSetModes::MTBCmdSetModes_Synchronous, 500);
	}
...
void ZeissMTB_Erlangen::setPositionRelativeX(double positionX) {
	bool success{ false };
	if (m_stageX) {
		success = m_stageX->SetPosition(positionX + m_homePosition.x, "�m", MTBCmdSetModes::MTBCmdSetModes_Synchronous, 500);
	}
	calculateCurrentPositionBounds();
}
...
void ZeissMTB_Erlangen::setPositionRelativeY(double positionY) {
	bool success{ false };
	if (m_stageY) {
		success = m_stageY->SetPosition(positionY + m_homePosition.y, "�m", MTBCmdSetModes::MTBCmdSetModes_Synchronous, 500);
	}
	calculateCurrentPositionBounds();
}
...
void ZeissMTB_Erlangen::setPositionRelativeZ(double positionZ) {
	bool success{ false };
	if (m_ObjectiveFocus) {
		success = m_ObjectiveFocus->SetPosition(positionZ + m_homePosition.z, "�m", MTBCmdSetModes::MTBCmdSetModes_Synchronous, 500);
	}
	calculateCurrentPositionBounds();
}
...
POINT3 ZeissMTB_Erlangen::getPosition() {
	...
	double z{ 0 };
	if (m_stageX && m_stageY) {
		x = m_stageX->GetPosition("�m");
		y = m_stageY->GetPosition("�m");
	}
	if (m_ObjectiveFocus) {
		z = m_ObjectiveFocus->GetPosition("�m");
	}
	return POINT3{ x, y, z };
}

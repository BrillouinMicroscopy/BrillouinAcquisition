	...
	TEST_CLASS(TestUnwrap) {
		public:
			...
			TEST_METHOD(TestUnwrap2DFast) {
				...
				for (int x{ 0 }; x < dim_x; x++) {
					for (int y{ 0 }; y < dim_y; y++) {
						wrapped[x + y * dim_x] = std::fmod(wrapped[x + y * dim_x], 2.0 * PI) / (2*PI);
					}
				}
				...
				std::vector<unsigned char> mask(dim_x * dim_y, 0);
				m_unwrapper->unwrap2DWrappedFast(&wrapped[0], &output[0], &mask[0], dim_x, dim_y);
				...
				Assert::IsTrue(sum < 1e-3*dim_x*dim_y);
			}
	};
}